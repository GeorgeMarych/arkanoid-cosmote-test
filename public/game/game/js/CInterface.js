function CInterface(iScore) {
  ...
  this._init = function (iScore) {
    ...
    _oLifeText = new createjs.Text(
      "x" + LIFE,
      "normal " + 40 + "px " + PRIMARY_FONT,
      "#ffffff"
    );
  ...
  this.gameOver = function (iScore) {
    ...
    createjs.Tween.get(_oFade)
      .to({ alpha: 0.5 }, 1000, createjs.Ease.backOut)
      .call(function () {
  ...
  this.levelComplete = function (iLevel, iScore) {
    ...
    if (iLv < BRICKS_LEVEL_POSITION.length) {
      ...
      _oTextScoreLevelComplete = new CTLText(
        s_oStage,
        CANVAS_WIDTH / 2 - 300,
        CANVAS_HEIGHT / 2 - 150,
        600,
      ...
      createjs.Tween.get(_oTextScoreLevelComplete.getText())
        .to({ alpha: 1 }, 1000, createjs.Ease.bounceOut)
        .call(function () {
          var oSpriteButNext = s_oSpriteLibrary.getSprite("but_continue");
          _oButNextLevel = new CGfxButton(
            CANVAS_WIDTH / 2,
            CANVAS_HEIGHT / 2 + 300,
            oSpriteButNext,
            s_oStage
          );
          _oButNextLevel.addEventListener(
            ON_MOUSE_UP,
            s_oInterface._onButNextLevelRelease,
            this
          );
        });
      createjs.Tween.get(_oTextLevelComplete.getText()).to(
  ...
  this.finishGame = function (iScore) {
    ...
    createjs.Tween.get(_oFade).to({ alpha: 1 }, 1000, createjs.Ease.backOut);
    createjs.Tween.get(_oTextCongrat.getText()).to(
  ...
  this._onButPauseRelease = function () {
    ...
    _oPauseText = new createjs.Text(
      "Pause",
      "normal " + 70 + "px " + PRIMARY_FONT,
      "#ffffff"
    );
    ...
    _oButContinue.addEventListener(
      ON_MOUSE_UP,
  ...
  this.showNumLevel = function (iLevel) {
    ...
    oCurLevel = new createjs.Text(
      TEXT_LEVEL_UPPERCASE + " " + iLevel,
      "normal " + 100 + "px " + PRIMARY_FONT,
      COLOR_TEXT_LEVEL[iRand]
    );
    ...
    oCurLevelCont = new createjs.Text(
      TEXT_LEVEL_UPPERCASE + " " + iLevel,
      "normal " + 100 + "px " + PRIMARY_FONT,
      "#000000"
    );
    ...
    createjs.Tween.get(oCurLevelContainer)
      .to({ x: CANVAS_WIDTH * 0.5 }, 600, createjs.Ease.cubicOut)
      .call(function () {
        createjs.Tween.get(oCurLevelContainer)
          .wait(500)
          .to({ x: CANVAS_WIDTH + 100 }, 600, createjs.Ease.cubicIn)
          .call(function () {
